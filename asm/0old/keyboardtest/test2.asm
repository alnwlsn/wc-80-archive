;need this here to provide supporting functions that are already in RAM

;WC-80 COMPUTER BOOTLOADER - alnwlsn 2021
;to be stored at address 0x0000 in rom bank 0 (loads on boot)
org 0x00
ld a,0xC0    ;rom 0 -> slot 0 THIS SHOULD BE THE FIRST THING YOU DO WHEN SYSTEM BOOTS
out (0x70),a 
...
out (0x73),a
ld sp,0x3000 ;stack at the end of bank 0 (RAM), but out of the trs80 no ram area

ld a,0x2a    ;debug display "****"
out (0x74),a
out (0x75),a
out (0x76),a ;finish debug display
out (0x77),a


;BOOT MENU ************************************************************
bootMenuText: DB "SELCT BOOT?",0
call clear
ld hl,bootMenuText
call ioStringPrintL
bootMenuSelectLoop: ;read the keyboard
	call getDskyKey
	cp "M"
	jp z,dskyMonStartPoint ;DSKYMON
	cp "K"
	jp z,clockdisp8 ;Clock Demo
	cp "I"
	jp z,0x3000
	cp "J"
	jp z,0x8000
	cp 0x10 ;c is set for keys 0-f
	jp c,bootBank ;bootBank
	jr bootMenuSelectLoop
...
bootBank: ;fill address space with RAM, clear it, copy a rom bank into ram at Slot 0, and jump to 0x0000
	...
	ld c,a
	ex af,af'
	call ioPrintHex8 ;print the selected ram bank to the debug screen
	...
	ld a,0x25; ram bank 5 -> slot 1, no exram
	out (0x71),a
	ld a,0x26; ram bank 6 -> slot 2
	out (0x72),a
...
;address 0x4000 is here, the real program begins here
...
;read the key matrix (0 to 63, aka 0x00 to 0x3f in hex. 0x40 if no key)
seek 0x4040
org 0x4040
loop101:
	ld d,0
	...
	loop102:
		...
		ld a,1
		cp l
		jr nz,loop102 ;if 0, we've gone all the way around
	keyexitL:
		ld a,d ;d contains key matrix key
