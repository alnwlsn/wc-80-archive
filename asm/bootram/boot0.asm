...
bootMenu:
	...
	in a,(0x61) ;first, look for the dsky & 0 key held. A will be 0xff if it exists
	or 0b11000000 ;don't care about highest 2 bits
	cp 0xff
	ld a,0x02 ;this is our default boot selection
	jr nz,bootBank ;if no dsky, continue with our default boot selection
...
bootBank: ;fill address space with RAM, clear it, copy a rom bank into ram at Slot 0, and jump to 0x0000
	...
	ld hl,dskyMonMsgBlank ;clear the dsky display
	call ioStringPrintL
...
getDskyKey: ;returns A with key scancode (or 255 and z if no key pressed)
	...
	or 0b11000000 ;set & ignore the highest 2 bits
	ld c,a
	ld b,6
	getDskyKey_loop1:
	ld a,1
...
srCheck: ;poll the shared ram, look for changes in command, and act on any command received (non blocking)
	...
	ld a,(srCommand)
	cp b
	jp z, srCheckExit ;exit if command was not changed
	cp 0x10
	jr z, srReadMem
	cp 0x20
	jr z, srWriteMem
...
incbin "conway.bin" ;conway's game of life demo
